...
from data.base_dataset import NewsDataset, ReviewDataset, IMDBDataset, SST2Dataset, FoodDataset,\
    ReutersDataset, MSRvidDataset, ImagesDataset, MSRparDataset, HeadlinesDataset, CoronaDataset, AGNewsDataset, FakeNewsDataset
...
def get_base_dataset(data_name, tokenizer, split_ratio=1.0, seed=0, test_only=False, remain=False):
    ...
    if data_name == 'news':
        ...
        dataset = AGNewsDataset(tokenizer, test_only=test_only)
    elif data_name == 'fake':
        dataset = FakeNewsDataset(tokenizer, test_only=test_only)
    else:
...
def get_attention_keyword(dataset, attn_model, keyword_per_class=10):
    ...
    for _, (tokens, _) in enumerate(loader):
        ...
        with torch.no_grad():
            print(attn_model(tokens).size())
            attention_layers = attn_model(tokens)

        attention = attention_layers[0]  # attention of final layer (batch_size, num_heads, max_len, max_len) # 0 for distil bert
